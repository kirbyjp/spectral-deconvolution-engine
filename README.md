...
* **Tier 2: The Instrumented Single-Axis Rig (Intermediate / Robotics Club Level)**
  ...
  * *Minimum IMU Specification (Pending Measurement):* Minimum IMU sampling rate and noise-density requirements will be added once shutter-shock and stepper micro-vibration frequencies are empirically measured. See Section 5 for the governing Nyquist and noise-floor relations that determine these thresholds.
...
* **Known Performance Gaps vs. Laboratory-Grade Systems:**  
  This architecture is not expected to match dedicated laboratory spectrometers in absolute flux calibration accuracy, optical throughput, or immunity to atmospheric effects for ground-based astronomical use. Stating these limits explicitly scopes the claimed 80% performance figure rather than implying general equivalence. Additionally, significant radial or transverse motion components introduce a spatially-variant (non-stationary) blur kernel due to perspective foreshortening, which a single global motion vector cannot fully correct; Section 6's multi-point sampling method provides a partial, empirically-grounded mitigation for this limitation.
